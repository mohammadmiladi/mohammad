    ...
    <div class="cardStyle">
      <b-row class="no-gutters justify-content-around align-items-center">
        ...
        <b-col cols="9" md="9" class="d-flex flex-column justify-content-evenly align-items-start">
          <div class="cardInfo">
            <div class="ml-1">شماره کارت:</div>
            <div>{{ sliceCardNumbers(bankData.cardNumber) }}</div>
          </div>
          <div class="cardInfo">
          ...
          <div class="cardInfoAndCopy w-100">
            ...
            <div class="d-flex">
              <div class="ml-2">
                <img @click="showSnack(bankData.cardNumber)" class="cursorPointer" src="@/assets/images/copy-green.svg" width="20px" />
              </div>
              <div class="ml-1">
...
export default {
  ...
  methods: {
    ...
    deleteCard(cardID) {
      ProfileDataProvider.deleteCard(cardID)
      setTimeout(() => {
        this.$refs['deleteCard'].hide()
        ////////// Show Card API Call //////
        ProfileDataProvider.getCard()
          .then((res) => {
            this.$store.dispatch('profile/setCardsData', res.data.data)
          })
          .catch((err) => {
