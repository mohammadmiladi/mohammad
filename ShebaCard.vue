    ...
    <div class="cardStyle">
      <b-row class="no-gutters justify-content-around align-items-center">
        ...
        <b-col cols="9" md="9" class="d-flex flex-column justify-content-evenly align-items-start">
          <div class="cardInfo">
            <div class="ml-1">شماره شبا:</div>
            <div>{{ sliceCardNumbers(bankData.number) }}</div>
          </div>
          <div class="cardInfo">
            <div class="ml-1">صادرکننده:</div>
            <div>{{ bankData.bankName }}</div>
          </div>
          <div class="cardInfoAndCopy w-100">
            <div class="d-flex">
              <div class="ml-1">وضعیت:</div>
              <div v-if="bankData.cardInquiryStatus === 0" style="color: #35a853; font-weight: 500">تایید شده</div>
              <div v-if="bankData.cardInquiryStatus === 1" style="color: #ea4234; font-weight: 500">تایید نشده</div>
              <div v-if="bankData.cardInquiryStatus === 2" style="color: #ffc940; font-weight: 500">در انتظار تایید</div>
            </div>
            <div class="d-flex">
              <div class="ml-2">
                <img @click="showSnack(bankData.number)" class="cursorPointer" src="@/assets/images/copy-green.svg" width="20px" />
              </div>
              <div class="ml-1">
                <img @click="openDeleteShebaModal" class="cursorPointer" src="@/assets/images/delete-red.svg" width="20px" />
              </div>
              <b-modal ref="deleteSheba" no-close-on-esc hide-footer centered size="md" hide-header-close cancel-title>
                ...
                <div v-if="bankData">
                  ...
                  <div>
                    <b-button @click="deleteSheba(bankData.id)" class="submitStyle2">بله</b-button>
...
export default {
  ...
  methods: {
    ...
    openDeleteShebaModal() {
      this.$refs['deleteSheba'].show()
    },
    closeModal() {
      this.$refs['deleteSheba'].hide()
    },
    deleteSheba(cardID) {
      ProfileDataProvider.deleteSheba(cardID)
      setTimeout(() => {
        this.$refs['deleteSheba'].hide()
        ////////// Show Sheba API Call //////
        ProfileDataProvider.getSheba()
          .then((res) => {
            this.$store.dispatch('profile/setShebaData', res.data.data)
            console.log(this.shebaData)
          })
          .catch((err) => {
